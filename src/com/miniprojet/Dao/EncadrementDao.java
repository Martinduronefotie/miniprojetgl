...
public class EncadrementDao extends Dao {
    ...
    public void updateEncadrement(Encadrement enc) {
        try {
            ...
            pst = super.getCon().prepareStatement("UPDATE encadrement SET themes_id_theme = ?,enseignant_id_enseignant = ?,etat_encadrement = ? type_encadrement = ?, WHERE id_encadrement = ?");
            pst.setInt(1, enc.getTheme().getId());

            pst.setInt(2, enc.getEnseignant().getId_enseignant());
            pst.setString(3, enc.getEta_encadrement());
            pst.setString(2, enc.getType_encadrement());
    ...
    public List<Encadrement> Allencadrement() {
        ...
        try {
            ...
            pst = super.getCon().prepareStatement("SELECT * FROM encadrement,themes,enseignant where themes_id_theme=id_theme and enseignant_id_enseignant=id_enseignant");
            rs = pst.executeQuery();
            while (rs.next()) {
